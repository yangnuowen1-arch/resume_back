...
func ConnectDB(cfg *config.Config) *gorm.DB { //传入配置对象，返回一个 GORM 数据库连接对象
	...
	if err := ensureScreeningResultVisibilityColumns(database); err != nil {
		log.Fatalf("筛选任务可见性字段初始化失败: %v", err)
	}
	if err := ensureScreeningResultRequirementsColumn(database); err != nil {
		log.Fatalf("筛选任务要求比对字段初始化失败: %v", err)
	}
...
func ensureScreeningResultVisibilityColumns(database *gorm.DB) error {
	return database.Exec(
		`ALTER TABLE screening_results ADD COLUMN IF NOT EXISTS started_at TIMESTAMP WITHOUT TIME ZONE;
	ALTER TABLE screening_results ADD COLUMN IF NOT EXISTS finished_at TIMESTAMP WITHOUT TIME ZONE;
	ALTER TABLE screening_results ADD COLUMN IF NOT EXISTS retry_count INTEGER NOT NULL DEFAULT 0;
	ALTER TABLE screening_results ADD COLUMN IF NOT EXISTS last_error_at TIMESTAMP WITHOUT TIME ZONE`,
	).Error
}

func ensureScreeningResultRequirementsColumn(database *gorm.DB) error {
	return database.Exec(
		`ALTER TABLE screening_results ADD COLUMN IF NOT EXISTS requirements JSONB`,
	).Error
}
